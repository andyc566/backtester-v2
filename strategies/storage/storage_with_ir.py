...
def apply_discount(df, risk_free_rates):
    df['MonthKey'] = df['Date'].dt.strftime('%Y-%m')
    df['DayOfMonth'] = df['Date'].dt.day
    df['DiscountFactor'] = df.apply(
        lambda row: (1 + risk_free_rates[row['MonthKey']] / 100) ** (-row['DayOfMonth'] / 30), axis=1
    )
...
def optimize_storage(config, risk_free_rates):
    ...
    df = apply_discount(df, risk_free_rates)
    ...
    for i in days:
        ...
        if month in config.winter_months:
            prob += injections[i] == 0
            prob += withdrawals[i] <= config.withdrawal_rates[-1]
        else:
            prob += withdrawals[i] == 0
            prob += injections[i] <= config.injection_rates[0]
